...
function db_new($dbconf) {
	...
	if($dbconf) {
		...
		switch ($dbconf['type']) {
			// db_mysql 已移植 mysqli; 无 mysqli 时回退 PDO, 老站配置无损 (Xiuno BBS 5.0)
			case 'mysql':
			case 'mysqli':
				$conf = isset($dbconf[$dbconf['type']]) ? $dbconf[$dbconf['type']] : $dbconf['mysql'];
				if(function_exists('mysqli_connect')) {
					$db = new db_mysql($conf);
				} elseif(extension_loaded('pdo_mysql')) {
					$db = new db_pdo_mysql($conf);
				} else {
					// 无任何可用驱动没有继续运行的意义，静默返回 FALSE 会让报错沉没在后续查询里
					exit('缺少 mysqli 扩展，且无 pdo_mysql 可回退，请安装 php-mysqli');
				}
				break;
